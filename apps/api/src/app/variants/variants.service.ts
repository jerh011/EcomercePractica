...
import type { AttributeSummaryDto } from '@ecomercepractica/shared/contracts/common/dto/output/entity-summary.dto';
import { CreateVariantDto } from '@ecomercepractica/shared/contracts/variants/dto/input/create-variant.dto';
import type { VariantAttributeValueInputDto } from '@ecomercepractica/shared/contracts/variants/dto/input/variant-attribute-value.dto';
import {
...
import { ToggleVariantStatusDto } from '@ecomercepractica/shared/contracts/variants/dto/input/toggle-variant.dto';
...
} from '@ecomercepractica/shared/contracts/variants/dto/output/list-variants-result.dto';
import { VariantDto } from '@ecomercepractica/shared/contracts/variants/dto/output/variant.dto';
import { VariantDimensionsDto } from '@ecomercepractica/shared/contracts/variants/dto/variant-dimensions.dto';
import { isUuid } from '@ecomercepractica/shared/validations';
...
export class VariantsService {
  ...
  async createVariant(
    ...
    try {
      const row = (await this.prisma.client.variant.create({
        data: {
  ...
  async listVariants(
    ...
    try {
      const [rows, totalCount] = await Promise.all([
        ...
        this.prisma.client.variant.count({ where }),
      ]);
      ...
      return {
        variants: await this.mapVariants(rows as VariantWithRelations[]),
        pagination: {
          offset: {
      ...
      const row = (await this.prisma.client.$transaction(async (tx: any) => {
        await tx.variant.update({
          ...
          data: {
            ...
            offerStart: merged.offerStart,
            offerEnd: merged.offerEnd,
  ...
  async deleteVariant(id: string): Promise<{ variant: VariantDto }> {
    ...
    try {
      const stockMap = await this.inventoryService.loadVariantStockMap([
        current.id,
      ]);
      await this.prisma.client.variant.update({
        where: { id },
  ...
  private async mapVariants(
    ...
    return rows.map((row) =>
      this.toVariantResponse(row, globalAttributes, stockMap.get(row.id) ?? 0),
    );
  }

  private async mapVariant(row: VariantWithRelations): Promise<VariantDto> {
    const globalAttributes = await this.loadGlobalAttributes();
    const stockMap = await this.inventoryService.loadVariantStockMap([row.id]);
    return this.toVariantResponse(
      row,
      globalAttributes,
      stockMap.get(row.id) ?? 0,
    );
  }
  ...
  private async loadGlobalAttributes(): Promise<VariantAttributeSummary[]> {
    const rows = await this.prisma.client.attribute.findMany({
      where: {
        deletedAt: null,
        isActive: true,
        appliesToAll: true,
  ...
  private async ensureUniqueAttributeCombination(
    ...
    // Definir un tipo para los elementos de rows
    type VariantAttributeValueItem = {
      attributeId: string;
      value: string;
    };

    type VariantWithAttributeValues = {
      id: string;
      attributeValues: VariantAttributeValueItem[];
    };

    const duplicate = rows.some((row: VariantWithAttributeValues) => {
      const current = row.attributeValues
        .map(
          (item: VariantAttributeValueItem) =>
      ...
      return current.every(
        (value: string, index: number) => value === normalized[index],
      );
    });
