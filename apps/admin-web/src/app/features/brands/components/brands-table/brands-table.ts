...
import { TruncateText } from '@shared/component/truncate-text/truncate-text';
...
export class BrandsTable {
  ...
  // ✅ CORREGIDO: snapshot debe mapear los datos, no los records completos
  private readonly snapshot = computed(() =>
    this.data().map((record) => structuredClone(record.data)),
  );

  logoUrl(row: { logoUrl?: string | string[] }): string {
    return Array.isArray(row.logoUrl)
      ? (row.logoUrl[0] ?? '')
      : (row.logoUrl ?? '');
  }
  ...
  // ✅ CORREGIDO: Usar la snapshot correctamente
  toggleBrandStatus(data: EntityData, newValue: boolean): void {
    const snapshotData = this.snapshot().find(
      (r) => r._recordKey === data._recordKey,
    );
    const record = this.data().find(
      (r) => r.data._recordKey === data._recordKey,
    );

    if (!snapshotData || !record) {
      this.service.brandStatusChange$.error(
        new Error(
          `No se encontró el registro para la marca con _recordKey: ${data._recordKey}`,
        ),
