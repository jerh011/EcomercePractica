...
import { BrandsTableService } from '@brands/components/brands-table/brands-table.service';
import { BrandActionsService } from '@brands/services/brand-actions/brand-actions.service';
import { ToastService } from '@shared/services/toast/toast.service';
import { RegisterBrandStrategy } from '@brands/components/brand-overview-actions/types';
import { DialogService } from '@shared/services/toast/dialog/dialog.service';
import { RegisterBrandDialog } from '@brands/dialogs/register-brand/register-brand.dialog';
...
export class BrandsPage implements OnInit {
  ...
  private handleMultiplpeBrandCreation(): void {
    this.router.navigate(['catalogs/brands/bulk-registration']);
  }
  ...
  private editBrand(record: BrandRecord): void {
    this.router.navigate(['catalogs/brands/edit', record.data._recordKey]);
  }
  ...
  private deleteBrand(record: BrandRecord): void {
    ...
    this.confirmBrandDeletion(brand)
      .pipe(filter(Boolean))
      .subscribe(() =>
        this.executeBrandDeletion(record.data._recordKey, brand),
      );
  }
  ...
  private registerStatusChangeHandler(): void {
    this.tableService.brandStatusChange$.subscribe({
      next: ({ snapshot, newValue }) =>
        this.handleStatusChange(snapshot, newValue),
    });
  }
  ...
  private handleStatusChange(snapshot: any, newValue: boolean): void {
    ...
    this.brandActionsService
      ...
      .subscribe({
        ...
        error: () =>
          this.onStatusChangeError(snapshot.data._recordKey, newValue, brand),
      });
  }

  private onStatusChangeSuccess(brand: Brand | undefined): void {
    this.toast.showSuccess(
      `Estado de la marca ${brand?.name} actualizado correctamente`,
    );
  }
  ...
  private registerVisibleInMenuChangeHandler(): void {
    this.tableService.brandVisibleInMenuChange$.subscribe({
      next: ({ snapshot, newValue }) =>
        this.handleVisibilityChange(snapshot, newValue),
    });
  }
  ...
  private handleVisibilityChange(snapshot: any, newValue: boolean): void {
    ...
    this.brandActionsService
      ...
      .subscribe({
        ...
        error: () =>
          this.onVisibilityChangeError(
            snapshot.data._recordKey,
            newValue,
            brand,
          ),
      });
  ...
  private onVisibilityChangeError(
    ...
    this.toast.showError(
      `Error al actualizar la visibilidad en menú: ${brand?.name}`,
    );
  }
  ...
  private revertBrandVisibility(id: string, failedValue: boolean): void {
    this.brands.update((brands) =>
      brands.map((b) =>
        b.id === id ? { ...b, visibleInMenu: !failedValue } : b,
      ),
    );
  }
