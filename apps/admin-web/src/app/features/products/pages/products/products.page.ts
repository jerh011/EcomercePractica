...
import { ProductsTableService } from '@products/components/products-table/products-table.service';
import { ProductOverviewActions } from '@products/components/product-overview-actions/product-overview-actions';
import { RegisterProductStrategy } from '@products/components/product-overview-actions/types';
import { DialogService } from '@shared/services/toast/dialog/dialog.service';
import { RegisterProductDialog } from '@products/components/dialogs/register-product/register-product.dialog';
import { DIMENSIONS } from '@shared/constants/dimensions.consts';
import { Router } from '@angular/router';
import { ProductActionsService } from '@products/services/product-actions/product-actions.service';
import { ToastService } from '@shared/services/toast/toast.service';
import {
    ProductFeaturedChange,
    ProductStatusChange,
    ProductTableActionEvent,
} from '@products/components/products-table/types';
import { ProductDetailsDialog } from '@products/components/dialogs/product-details/product-details.dialog';
// import { Slot } from "@shared/directives/slot/slot";

@Component({
  selector: 'ecom-products.page',
...
export class ProductsPage implements OnInit {
  ...
  rows = computed(() =>
    this.products().map((product) => toPersistedRecord(product)),
  );
  ...
  private readonly pageSize = computed(
    () => this.service.paginationParams().pageSize,
  );
  pagination = computed(() =>
    createPagination({
      showPagination: true,
  ...
  private registerProductActionHandler(): void {
    this.tableService.actionProduct$.subscribe((event) =>
      this.handleProductAction(event),
    );
  }
  ...
  private handleEditProduct(productId: Product['id']): void {
    this.router.navigate([`catalogs/products/${productId}/edit`]);
  }
  ...
  private toggleProductStatusHandler(event: ProductStatusChange): void {
    ...
    this.productActionsService
      ...
      .subscribe({
        next: () =>
          this.onStatusChangeSuccess(product, {
            ...product!,
            isActive: newValue,
          }),
  ...
  private toggleProductFeaturedHandler(event: ProductFeaturedChange): void {
    ...
    this.productActionsService
      ...
      .subscribe({
        next: () =>
          this.onFeaturedChangeSuccess(product, {
            ...product!,
            isFeatured: newValue,
          }),
  ...
  private onRegisterMultipleProducts(): void {
    this.router.navigate(['catalogs', 'products', 'bulk-registration']);
  }
